%% function run_for_oscillator_kymo
% generates kymographs for oscillator phenotype
...
t = p.t0:p.dt:p.tf;
 ...
 cell_type_list = {'pikl-:oscillator'};
 tic
    for k = 1
...
%--------------------------------------------------------------------------
    ...
    switch p.cell_type
        case 'WT'
            p.a = 1.5*0.25*10/10; % Ras activation by Actin: p.a; change if needed
            p.m = 0.25*6/10;  % Ras inhibition by Myosin: p.m; change if needed

         case 'pikl-:fan'           
            p.a = 5*0.25*10/10; % Ras activation by Actin: p.a; change if needed
            p.m = 1*0.25*6/10;  % Ras inhibition by Myosin: p.m; change if needed
            % p.c2 = 1.0*0.8640*p.scaling*p.scaling2;
           

         case 'pikl-:oscillator'            
            p.a = 0.5*0.25*10/10; % Ras activation by Actin: p.a; change if needed
            p.m = 0.5*0.25*6/10;  % Ras inhibition by Myosin: p.m; change if needed
         ...
         case 'OE1'
            ...
            % p.DPKB  = 1.1*3*0.03*p.scaling*p.scale_spatial*p.scaling2;
            % p.DPIP2 = 1.1*1*0.03*p.scaling*p.scale_spatial*p.scaling2;
        
         case 'OE2'
            p.a = 0.25*10/10; % Ras activation by Actin: p.a; change if needed
            p.m = 0.25*6/10;  % Ras inhibition by Myosin: p.m; change if needed

         case 'lat'
            p.a = 5*0.25*10/10; % Ras activation by Actin: p.a; change if needed
            p.m = 0.25*6/10;  % Ras inhibition by Myosin: p.m; change if needed
            p.c2 = 2*0.8640*p.scaling*p.scaling2;

         case 'big protrusion'
            p.a = 0.25*10/10; % Ras activation by Actin: p.a; change if needed
            p.m = 0.25*6/10;  % Ras inhibition by Myosin: p.m; change if needed

         case 'pikl-:oscillator2'            
            p.a = 2*0.5*0.25*10/10; % Ras activation by Actin: p.a; change if needed
            p.m = 2*0.5*0.25*6/10;  % Ras inhibition by Myosin: p.m; change if needed
...
%--------------------------------------------------------------------------
    switch p.cell_type
            ...
            case 'pikl-:fan'
                % lower initial amount of PIP5K
                p.PIP5K_total = 0.5*10/126;
                p.PIP5K_mem0  = 0.5*9/126;

            case 'pikl-:oscillator'
                p.PIP5K_total = 0.02*10/126;
                p.PIP5K_mem0  = 0.02*9/126;
            ...
            case 'OE2'
                % higher initial amount of PIP5K
                p.PIP5K_total = 2*10/126;
                p.PIP5K_mem0  = 2*9/126;
            ...
            case 'pikl-:oscillator2'
                p.PIP5K_total = 0.1*10/126;
                p.PIP5K_mem0  = 0.1*9/126;
    end


    % p.PIP5K_total = 10/p.Np;
    % p.PIP5K_mem0 = 9/p.Np;
...
%% SDE toolbox
    ...
    timeRange = t;
    problem = 'yudeng3'; % name of the sde file
...
hold on
imagesc(T-T_initial,p.x,circshift(Ras',d,1))
% imagesc(T-T_initial,p.x,circshift(smoothdata(Ras)',d,1))
% surf(T-T_initial,p.x,circshift(Ras',d,1),'edgecolor','interp')
colorbar
clim([0 0.2])
% colormap(gray)
...
% set(gca, 'fontweight','n','linewidth',1,'fontsize',16)
% hold on
% % imagesc(T-T_initial,p.x,circshift(smoothdata(PIP2)',d,1))
% surf(T-T_initial,p.x,circshift(smoothdata(PIP2,"gaussian")',d,1),'edgecolor','interp')
% % shading interp
% colorbar
% clim([0 1])
% mycolormap = magma(100);
...
% surf(T-T_initial,p.x,circshift(smoothdata(PKB,"gaussian")',d,1),'edgecolor','interp')
% shading interp
colorbar
clim([0 1])
colormap(gray)
...
% % surf(T-T_initial,p.x,circshift(Actin',d,1),'edgecolor','interp')
% colorbar
% colormap(gray)
% mycolormap = magma(100);
% colormap(mycolormap)
