function xhat = SDE_euler_deb(bigtheta,PROBLEM,OWNTIME,NUMDEPVARS,NUMSIM,SDETYPE,param)
%
...
[t, xstart] = feval([PROBLEM, '_sdefile'],OWNTIME(1),[],'init',bigtheta,SDETYPE,NUMDEPVARS,NUMSIM,param); % Debojyoti 06/20/2025
...
%     waitbar((j-1)/(N-1)); % commented Debojyoti
    ...
    % Winc(1 : NUMDEPVARS * NUMSIM) = sqrt(h)*randn(1,NUMDEPVARS * NUMSIM); % the Wiener increment(s) dWj;
    ...
    [f,g] = feval([PROBLEM, '_sdefile'], t, x, [], bigtheta,SDETYPE,NUMDEPVARS,NUMSIM,param);  % Debojyoti 06/20/2025
