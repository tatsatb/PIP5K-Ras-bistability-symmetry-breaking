function [t,y,schemeData]=odeCFL1Shi(schemeFunc,tspan,y0,options,schemeData)
    % odeCFL1: integrate a CFL constrained ODE (eg a PDE by method of lines).
    %
    % [t,y,schemeData]=odeCFL1Shi(schemeFunc,tspan,y0,options,schemeData)
    %
...
% 
            ...
            % Reinitiate the data to signed distance function
            phi = reshape(y(:,1), g.shape);
            phi = reinitOnTheFly(g, phi, 0.1, 'low');
            y(:,1) = phi(:);

            % Update the step
            steps = steps + 1;

            % Monitor the time of every step
            fprintf('\nJust finished time point %g s, computation time %g seconds',...
            t, cputime-tStart);
            disp('------------------------------------------------------')
...
end %function [t,y,schemeData]=odeCFL1Shi